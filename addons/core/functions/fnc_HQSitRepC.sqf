...
if (isNil ("RydHQC_AmmoBoxes")) then 
	...
	if !(isNil "RydHQC_AmmoDepot") then 
		{
...
while {true} do
	...
	if (isNil ("RydHQC_RCASB")) then {RydHQC_RCAS = []};
	_HQ setVariable ["RydHQ_RCAS",RydHQC_RCAS];
	if (isNil ("RydHQC_RCAPB")) then {RydHQC_RCAP = []};
	...
	if (isNil "RydHQC_Obj1") then {RydHQC_Obj1 = createTrigger ["EmptyDetector", leaderHQB]};
	if (isNil "RydHQC_Obj2") then {RydHQC_Obj2 = createTrigger ["EmptyDetector", leaderHQB]};
	if (isNil "RydHQC_Obj3") then {RydHQC_Obj3 = createTrigger ["EmptyDetector", leaderHQB]};
	if (isNil "RydHQC_Obj4") then {RydHQC_Obj4 = createTrigger ["EmptyDetector", leaderHQB]};
