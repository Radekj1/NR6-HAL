...
if (isNil ("RydHQG_AmmoBoxes")) then 
	...
	if !(isNil "RydHQG_AmmoDepot") then 
		{
...
while {true} do
	...
	if (isNil ("RydHQG_RCASB")) then {RydHQG_RCAS = []};
	_HQ setVariable ["RydHQ_RCAS",RydHQG_RCAS];
	if (isNil ("RydHQG_RCAPB")) then {RydHQG_RCAP = []};
	...
	if (isNil "RydHQG_Obj1") then {RydHQG_Obj1 = createTrigger ["EmptyDetector", leaderHQB]};
	if (isNil "RydHQG_Obj2") then {RydHQG_Obj2 = createTrigger ["EmptyDetector", leaderHQB]};
	if (isNil "RydHQG_Obj3") then {RydHQG_Obj3 = createTrigger ["EmptyDetector", leaderHQB]};
	if (isNil "RydHQG_Obj4") then {RydHQG_Obj4 = createTrigger ["EmptyDetector", leaderHQB]};
